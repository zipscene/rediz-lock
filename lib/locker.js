...
class Locker {
		...
		return this.scriptWaiter.promise.then(() => {
			return pasync.eachSeries(keys, (key) => {
				...
				return this._retryUntilTimeOut(
						this.redizClient.runScript('readLock',
	...
	_unlockRead(keys) {
		return pasync.eachSeries(keys.reverse(), (key) => {
			return this.redizClient.runScript('readLockRelease', this.prefix + ':read:' + key);
		});
	}
	...
	_retryUntilTimeOut(promise, timeout, key, type) {
		...
		let retry = (waitTime) => {
			return promise.then( (result) => {
				...
				return timeoutPromise.then( (time) => {
					return retry(time);
				});
			});
		};
...
// TODO :: WRITE LOCK TOKEN STUFF!!
	...
	writeLock(keys, options = {}) {
		...
		return this.scriptWaiter.promise.then(() => {
			...
			return new RWLock(this, lockedKeys, true);
		}).catch( (error) => {
			if (lockedKeys.length) {
				this._unlockWrite(lockedKeys).then( () => {
					throw error;
				});
			} else {
				throw error;
	...
	_unlockWrite(keys) {
		return pasync.eachSeries(keys.reverse(), (key) => {
			let client = this.redizClient.shard(key);
			return client.runScript('writeLockRelease', this.prefix + ':write:' + key);
		});
	}
