...
class Locker extends LockerBase {
	...
	async readLock(key, options = {}) {
		...
		return await profiler.run('#readLock', async() => {
			...
			try {
				...
				return new RWLock(this, key, token, distributedShard, false, heartbeatInterval, heartbeatTimeout);
	 ...
	 *   @param {Boolean} [options.distributed=false] - If true, enables a distributed write lock.
	 * @return {Promise{RWLock}}
	 */
	async writeLock(key, options = {}) {
		if (options.distributed) {
			// Perform a distributed write lock instead
		...
		return await profiler.run('#writeLock', async() => {
			...
			if (lostConflictResolution) {
				...
				return new RWLock(this, key, token, null, true, heartbeatInterval, heartbeatTimeout);
			}
		});
	}
