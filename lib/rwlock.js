...
class RWLock {
	 ...
	 */
	constructor(
		locker, key, token, distributedShard = null,
		isWriteLock = false, heartbeatInterval = 1000, heartbeatTimeout = 5
	) {
		this.locker = locker;
		this.key = key;
		...
		this.isLocked = true;
		this.token = token;
		this.distributedShard = distributedShard;
	...
	_getShardKey() {
		if (typeof this.distributedShard === 'number') return this.distributedShard;
		return this.key;
	}
	...
	_startHeartbeat() {
		...
		this.heartbeatHandle = setInterval(() => {
			let client = this.locker.redizClient.shard(this._getShardKey());
			let promise;
			if (this.isWriteLock) {
	...
	_startDistributedLockFlagCheckTimer(timerDelay, options = {}) {
		this._distributedLockFlagCheckTimer = setTimeout(async() => {
			if (this.isLocked) {
				try {
					let newDelay = await this.locker._checkAndSetDistributedLockFlag(
						this.key,
						options
					);
					if (newDelay > 0) {
						this._startDistributedLockFlagCheckTimer(newDelay, options);
					}
				} catch (ex) {
					pasync.abort(ex);
				}
	...
	_stopDistributedLockFlagCheckTimer() {
		if (this._distributedLockFlagCheckTimer) {
			clearTimeout(this._distributedLockFlagCheckTimer);
			delete this._distributedLockFlagCheckTimer;
		}
	}
	...
	forceRelease() {
		this._stopHeartbeat();
		this._stopDistributedLockFlagCheckTimer();
		if (!this.isLocked) return Promise.resolve();
		this.isLocked = false;
		if (!this.isWriteLock) {
			let client = this.locker.redizClient.shard(this._getShardKey(), { downNodeExpiry: 0 });
